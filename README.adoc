...
Rememember that though the backend service could implement it, it's more efficient to factor this feature in the API Gateway than to implement it in every service.
...
Authentication is tied to an identity.
In Apache APISIX, we call an identity a `Consumer`.
On the dashboard, display the existing `Consumer` objects by clicking on the btn:[Consumer] menu item on the left side:
there should be none.

Click on btn:[Create].

image::dashboard-create-new-consumer.png[]

In the next screen, give the new `Consumer` a name, _e.g._, `johndoe`.

image::dashboard-consumer-detail.png[]

Click btn:[Next] until the end of the wizard.
The new `Consumer` should now appear in the list.
