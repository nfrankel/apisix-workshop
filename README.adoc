...
[NOTE]
====
The dashboard sends Admin API calls under the cover.
...
Logs are another widespread pillar of Observability.
...
We can now explore data.
. Find the menu:Explore[] on the left side or click on `http://localhost:3000/explore`
. Select the _Loki_ data source in the upper menu
+
.Grafana Explore - Loki data source
image::loki-explore.png[Grafana Explore - Loki data source]
. Before going further, we need to filter the query.
Select `job` as the key and `apisix` as the (only) value
. Click on the btn:[Run query] button in the upper right corner
+
.Grafana Explore - Loki query
image::loki-query.png[Grafana Explore - Loki query]
. If you want, you can select a log line below the graph and switch on btn:[Prettify JSON]
+
.Grafana Explore - Log detail
image::loki-log-json-detail.png[Grafana Explore - Log detail]
+
The above is the default JSON sent by Apache APISIX.
