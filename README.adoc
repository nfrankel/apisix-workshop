...
== More plugins goodness

As mentioned above, lots of APISIX features are implemented via plugins.
In this section, we are going to browse and use a couple of them across different use cases.
...
If you want to add it again, you'll need to configure it from scratch.
You can remove a `Plugin` on a `Route` in two ways:
** Use `PUT` to completely replace the existing `Route`
** Use `PATCH` to only remove the selected plugin by setting it to `null`
...
[source,bash]
----
include::{commandsdir}/8_disable_plugin.sh[]
----
