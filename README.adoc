...
++++
<a href="https://github.com/nfrankel/apisix-workshop/fork" target="_blank"><img decoding="async" width="149" height="149" src="https://github.blog/wp-content/uploads/2008/12/forkme_right_green_007200.png?resize=149%2C149" class="attachment-full size-full" alt="Fork me on GitHub" loading="lazy" data-recalc-dims="1" style="position:fixed;top:0px;right:0px"></a>
++++
...
[source,bash]
----
curl http://localhost:9180/apisix/admin/routes/1/plugins/consumer-restriction -H 'X-API-KEY: edd1c9f034335f136f87ad84b625c8f1' -X PUT -d '
{
...
Observability is the process of getting constant insights into the system's behavior.
...
Let's remove the `key-auth` plugin we added in the previous section.
Read the https://docs.api7.ai/apisix/reference/plugin-common-configurations#_metadisable[appropriate documentation^] and disable the `auth-plugin` on the `Route` with ID `1`.
...
[source,bash]
----
include::{commandsdir}/12_disable_plugin.sh[]
----

<1> Path to the `key-auth` plugin, _i.e._, `/routes/1/plugins/key-auth`
<2> The body set the `disable` meta parameter to `true`
====

Send a request to the endpoint without an `apiky` header; it should work because the `auth-key` plugin is now disabled.
